...
# personal details
    ...
    # update the status
        if float(self.down) >= PROMISED_DOWNLOAD and float(self.up) >= PROMISED_UPLOAD :
            kudos_tweet = f"@RailWireIndia, kudos! for providing {self.down}down/{self.up}up :)"
    
            api.update_status(status = kudos_tweet)
            print("status updated")

        elif float(self.down) < PROMISED_DOWNLOAD or float(self.up) < PROMISED_UPLOAD :
            grumpy_tweet = f"@RailWireIndia, why is my internet speed {self.down}down/{self.up}up when I pay for {PROMISED_DOWNLOAD}down/{PROMISED_UPLOAD}up?"
    
            api.update_status(status = grumpy_tweet)
            print("status updated")
